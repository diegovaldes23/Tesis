...
"""
=============================================================================
  Video Swin Transformer (REAL) — Detección de Anomalías en Video
  Dataset  : UCF-Crime (processed/index_clips.csv)
  Pipeline : Video → VideoSwin3D → embedding → MLP → pred binaria

  Implementación: torchvision.models.video.swin3d_b (oficial PyTorch)
  Checkpoint: Swin3D_B_Weights.KINETICS400_IMAGENET22K_V1
  Preentrenamiento: ImageNet-22K → Kinetics-400
  Embedding dim: 1024 (Swin-Base último stage)
  T = 32 frames por clip

  3 experimentos (igual que I3D y TimeSformer):
    1. BASELINE  : VideoSwin congelado + MLP (sobre embeddings)
    2. LORA+MLP  : VideoSwin+LoRA end-to-end + MLP (lr=1e-4)
  ...
  Configuración LoRA (calibrada sobre I3D):
    ...
    Capas objetivo: attn.proj dentro de SwinTransformerBlock3d
                    (torchvision accede a qkv.weight directo, no via módulo)

  CHECKPOINTS DE FASE:
    rm processed/realswin_results/.phase_emb_done
    rm processed/realswin_results/.phase_baseline_done
    rm processed/realswin_results/.phase_lora_done
    rm processed/realswin_results/.phase_solo_lora_done
    rm processed/realswin_results/.phase_*  → todo desde cero

  Salidas en processed/realswin_results/:
    realswin_baseline_best.pth
    realswin_lora_best.pth
    realswin_solo_lora_best.pth
    realswin_results.json
    realswin_figures/
=============================================================================
Ejecutar:
    screen -S realswin
    CUDA_VISIBLE_DEVICES=0 python3 videoswin_real.py 2>&1 | tee realswin.log
    Ctrl+A D
    tail -f realswin.log
=============================================================================
"""
...
OUT_DIR    = Path("processed/swin_final_r16a16d024")
FIG_DIR    = OUT_DIR / "realswin_figures"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ── Lectura desde disco LOCAL (mucho mas rapido que el NFS) ───────────────────
PATH_NFS   = "/home/DIINF/dvaldes/tesis/UCF_Crime"
PATH_LOCAL = "/tmp/dvaldes/UCF_Crime"      # pon None para leer del NFS
...
MLP_CFG = {
    ...
    "patience":   8,
    "batch_size": 64,
    "batch_vid":  4,    # Swin3D es muy pesado en VRAM — mantener en 4
    "seed":       42,
}

# ── Configuración LoRA ───────────────────────────────────────────────────────
# DIAGNÓSTICO CONFIRMÓ (diagnostico_lora.py):
#   torchvision accede a qkv.weight Y proj.weight directamente via F.linear,
#   sin invocar el módulo → LoRALinear.forward() nunca se ejecuta para ellos.
#
#   Estrategias que SÍ funcionan (forward() invocado + grad_fn garantizado):
#     ✅ Estrategia 3: MLP fc linears ('0','3') — 48 capas, 481,280 params
#        → Capas feed-forward dentro de cada SwinTransformerBlock3d
#        → Son llamadas normalmente como módulo → grad fluye correctamente
#        → Target FFN es estándar en literatura LoRA (e.g. Hu et al. 2022)
#     ✅ Estrategia 2: hook en attention block — 24 bloques, 96,256 params
#        → Alternativa si se quieren menos params (ver diagnostico_lora.py)
#
# Se usa estrategia 3 por consistencia con I3D y TimeSformer del pipeline.
LORA_CFG = {
    "rank":         16,
    "alpha":        16,
    "lora_dropout": 0.24,
    "target_mods":  ("0", "3"),   # fc1 y fc2 del MLP interno de cada bloque
    # NOTA: apply_lora filtra solo capas dentro de rutas ".mlp." para evitar
    # reemplazar accidentalmente otras capas con índice "0" o "3".
}

# ── Parámetros de video ───────────────────────────────────────────────────────
T           = 32
IMG_SIZE    = 224
NUM_WORKERS = 16      # más workers → DataLoader no bottleneckea la GPU
# Normalización del checkpoint oficial de torchvision
IMG_MEAN = np.array([0.4850, 0.4560, 0.4060], dtype=np.float32)
IMG_STD  = np.array([0.2290, 0.2240, 0.2250], dtype=np.float32)
EMB_DIM  = 1024   # Swin3D-Base: 128 * 2^3 = 1024 en último stage
...
class ClipDataset(Dataset):
    def __init__(self, df, T=32, img_size=224):
        self.df       = df
        self.T        = T
...
# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 3 — Utilidades memmap y dataloaders
# ─────────────────────────────────────────────────────────────────────────────
...
def make_vid_loaders(df_tr, df_va, df_te, bs=4):
...
# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 4 — Carga de Video Swin 3D (torchvision oficial)
# ─────────────────────────────────────────────────────────────────────────────
def build_swin_backbone(freeze=True):
    """
    Carga Swin3D-Base preentrenado en Kinetics-400 via torchvision.
    Elimina la cabeza de clasificación y extrae el embedding de 1024 dims
    mediante GlobalAveragePooling sobre el último stage.
    """
    weights = Swin3D_B_Weights.KINETICS400_IMAGENET22K_V1
    model   = swin3d_b(weights=weights)

    # Eliminar la cabeza de clasificación — solo queremos el backbone
    model.head = nn.Identity()

    n_total = sum(p.numel() for p in model.parameters())
    print(f"  Swin3D-B cargado | parámetros: {n_total:,}")
...
def get_swin_embedding(model, x):
    """
    x: (B, C, T, H, W) — formato interno del ClipDataset
    Swin3D espera exactamente (B, C, T, H, W)
    Retorna: (B, 1024)
    """
    with torch.no_grad():
        out = model(x)   # (B, 1024) con head=Identity
    return out


# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 5 — LoRA para Video Swin 3D (reemplazando capas lineales qkv)
# ─────────────────────────────────────────────────────────────────────────────
class LoRALinear(nn.Module):
    """
    W + (α/r)·B·A — solo lora_A y lora_B son entrenables.

    FIX: Los pesos frozen del backbone se almacenan como register_buffer
    (NO como nn.Parameter) para evitar conflictos en el grafo de autograd
    cuando el MLP está congelado y solo LoRA debe entrenar.

    IMPORTANTE: torchvision accede a self.qkv.weight directamente en su
    código interno (swin_transformer.py línea ~305), por eso exponemos
    `.weight` y `.bias` como @property que apuntan al buffer.  Así:
      - torchvision puede leer .weight sin AttributeError
      - el tensor sigue siendo un buffer (requires_grad=False, sin grad)
      - lora_A / lora_B son los únicos Parameters → grad_fn correcto
    """

    def __init__(self, linear: nn.Linear, rank: int,
                 alpha: float, lora_dropout: float = 0.0):
        super().__init__()
        # ── Pesos base frozen → buffers ────────────────────────────────────────
        ...
        # ── Pesos LoRA entrenables ─────────────────────────────────────────────
        self.lora_A = nn.Parameter(
            torch.randn(rank, in_f, device=linear.weight.device) * 0.01)
        self.lora_B = nn.Parameter(
            torch.zeros(out_f, rank, device=linear.weight.device))

        self.dropout = nn.Dropout(p=lora_dropout) if lora_dropout > 0 \
                       else nn.Identity()

    # Propiedades para compatibilidad con torchvision que accede a .weight/.bias
    # directamente en su código fuente (no solo vía forward)
    @property
    def weight(self):
    ...
    def forward(self, x):
        # Rama base: buffer frozen, sin grad propio
        base = F.linear(x, self.base_weight, self.base_bias)
        # Rama LoRA: lora_A/lora_B tienen requires_grad=True →
        # el output tiene grad_fn incluso cuando x no requiere grad
        lora = F.linear(self.dropout(x), self.lora_A)
        lora = F.linear(lora, self.lora_B) * self.scale
        return base + lora


def apply_lora(model, rank, alpha, lora_dropout, target_mods=("0", "3")):
    """
    Aplica LoRA reemplazando capas Linear cuyo nombre hoja coincida con
    target_mods Y (si el target es numérico) cuya ruta completa contenga
    '.mlp.' para evitar reemplazar capas no deseadas.

    Diagnóstico confirmó que en torchvision Swin3D:
      - qkv, proj → accedidos via F.linear directo, forward() nunca invocado
      - fc del MLP interno ('0','3') → llamados como módulo → grad fluye ✅

    Las capas objetivo son: features.N.M.mlp.0 y features.N.M.mlp.3
    (fc1 y fc2 del feed-forward dentro de cada SwinTransformerBlock3d)
    """
    # Determinar si los targets son numéricos (MLP fc) → filtrar por ruta
    numeric_targets = {t for t in target_mods if t.isdigit()}
    named_targets   = {t for t in target_mods if not t.isdigit()}
    ...
    for name, module in model.named_modules():
        if not isinstance(module, nn.Linear):
            continue
        leaf = name.split(".")[-1]
        # Target con nombre textual (ej. "proj"): solo coincidencia por hoja
        if leaf in named_targets:
            to_replace.append(name)
        # Target numérico (ej. "0", "3"): solo dentro de rutas .mlp.
        elif leaf in numeric_targets and ".mlp." in name:
            to_replace.append(name)

    if len(to_replace) == 0:
        print("  [WARN] No se encontraron capas. Capas disponibles (primeras 30):")
        count = 0
        for n, m in model.named_modules():
            if isinstance(m, nn.Linear) and count < 30:
                print(f"    {n}  [{m.in_features}→{m.out_features}]")
                count += 1
        raise RuntimeError(
            "LoRA no encontró capas. Revisa LORA_CFG['target_mods'].")
...
def load_swin_frozen():
    return build_swin_backbone(freeze=True)


def load_swin_with_lora():
    model = build_swin_backbone(freeze=True)
    model, n_lora, n_total = apply_lora(
        model,
        rank=LORA_CFG["rank"],
    ...
    # Verificación de seguridad: confirmar que lora_A/lora_B requieren grad
    lora_params = [(n, p) for n, p in model.named_parameters()
                   if p.requires_grad]
    assert len(lora_params) > 0, \
        "ERROR: ningún parámetro LoRA tiene requires_grad=True"
    return model, n_lora, n_total


# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 6 — Clasificador MLP
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 7 — Extracción de embeddings (Baseline)
# ─────────────────────────────────────────────────────────────────────────────
def extract_embeddings(model, df, X_mm, y_mm, split_name):
    model.eval()
    loader = DataLoader(
    ...
    with torch.no_grad():
        for xb, yb in tqdm(loader, desc=f"  {split_name}", leave=True):
            emb = model(xb.to(DEVICE))   # Swin3D: (B, C, T, H, W) → (B, 1024)
            bs  = emb.shape[0]
            X_mm[offset:offset + bs] = emb.cpu().numpy().astype(X_mm.dtype)
...
# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 8 — FullPipeline y training loops
# ─────────────────────────────────────────────────────────────────────────────
class FullPipeline(nn.Module):
    """Swin3D (con LoRA) + MLP — forward sobre video crudo (C,T,H,W)."""
    ...
    def forward(self, x):
        # Swin3D espera (B, C, T, H, W) — mismo formato que ClipDataset
        emb = self.encoder(x)
        return self.mlp(emb)
...
def train_endtoend(full_model, tr_ldr, va_ldr, lr, epochs, patience,
                   ckpt_path, label):
    """Entrena LoRA+MLP end-to-end con AMP (float16) para mayor velocidad."""
    crit    = nn.BCEWithLogitsLoss()
...
def train_endtoend_solo_lora(full_model, tr_ldr, va_ldr, lr, epochs,
                              patience, ckpt_path, label):
    """
    Solo LoRA: MLP congelado (eval), solo LoRA aprende. Con AMP.
    """
    crit    = nn.BCEWithLogitsLoss()
    opt     = torch.optim.Adam(
        [p for p in full_model.parameters() if p.requires_grad], lr=lr)
        ...
        for xb, yb in tqdm(tr_ldr, desc="  train", leave=False):
            ...
            if ep == 1 and tr_loss == 0.0 and out.grad_fn is None:
                raise RuntimeError(
                    ...
                    "verifica que LoRALinear use register_buffer "
                    "y que lora_A/lora_B tengan requires_grad=True.")
...
# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 9 — Métricas operacionales
# ─────────────────────────────────────────────────────────────────────────────
def measure_operational_metrics(encoder, mlp, n_clips=100,
                                 frames_por_clip_original=32):
    encoder.eval()
    mlp.eval()
    ...
    ms_clip       = float(np.mean(times) * 1000)
    fps           = T / float(np.mean(times))
    ms_frame      = ms_clip / T
    res = {
        "latencia_ms_clip":  round(ms_clip,  2),
        "ms_frame":          round(ms_frame, 2),
        "fps":               round(fps,      1),
        "viable_realtime":   fps >= 30.0,
        "frames_por_clip":   T,
    }
    print(f"  FPS: {fps:.1f} | ms/frame: {ms_frame:.2f} | "
          f"Tiempo real: {'✔' if fps >= 30 else '✗'}")
    return res


# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 10 — Evaluación test
# ─────────────────────────────────────────────────────────────────────────────
@torch.no_grad()
...
# SECCIÓN 11 — Gráficos
# ─────────────────────────────────────────────────────────────────────────────
COLORS = {"BASELINE": "steelblue", "LORA+MLP": "tomato", "SOLO_LORA": "seagreen"}
PREFIX = "realswin"
...
def plot_training_curves(histories):
    ...
    fig.suptitle("Video Swin 3D — Curvas de entrenamiento",
    ...
    for cfg, hist in histories.items():
        ...
        if "val_auc" in hist:
            ...
            axes[1].axhline(hist["best_auc"], linestyle=":", color=c, alpha=0.4)
    axes[0].set(xlabel="Época", ylabel="BCE Loss", title="Curvas de pérdida")
    axes[0].legend(fontsize=8); axes[0].grid(alpha=0.3)
    axes[1].set(xlabel="Época", ylabel="AUC (validación)",
                title="AUC por época", ylim=(0.5, 1.0))
    axes[1].legend(fontsize=8); axes[1].grid(alpha=0.3)
    plt.tight_layout(); _save_fig("01_curvas_entrenamiento")
...
def plot_roc(test_results):
    ...
    for cfg, (probs, labels, _) in test_results.items():
        ...
        plt.plot(fpr, tpr, color=COLORS.get(cfg, "gray"),
                 label=f"{cfg} (AUC={roc_auc_score(labels, probs):.4f})")
    plt.plot([0, 1], [0, 1], "k--", label="Aleatorio")
    plt.xlabel("FPR"); plt.ylabel("TPR")
    plt.title("Curva ROC — Video Swin 3D (Test)")
    plt.legend(fontsize=9); plt.grid(alpha=0.3)
    _save_fig("02_curva_roc")
        ...
        for i in range(2):
            for j in range(2):
                ax.text(j, i, str(cm[i, j]), ha="center", va="center",
                        ...
                        fontsize=13, fontweight="bold")
    plt.tight_layout(); _save_fig("04_matrices_confusion")
...
def plot_scores(test_results):
    ...
    if len(cfgs) == 1: axes = [axes]
    fig.suptitle("Video Swin 3D — Distribución de scores (Test)",
                 fontsize=13, fontweight="bold")
    for ax, cfg in zip(axes, cfgs):
        probs, labels, _ = test_results[cfg]
    ...
    plt.tight_layout(); _save_fig("05_distribucion_scores")


# ─────────────────────────────────────────────────────────────────────────────
# SECCIÓN 12 — Orquestador principal
# ─────────────────────────────────────────────────────────────────────────────
def run_pipeline():
    ...
    paths = {
        "X_tr": OUT_DIR / "realswin_X_train.mmap",
        "y_tr": OUT_DIR / "realswin_y_train.mmap",
        "X_va": OUT_DIR / "realswin_X_val.mmap",
        "y_va": OUT_DIR / "realswin_y_val.mmap",
        "X_te": OUT_DIR / "realswin_X_test.mmap",
        "y_te": OUT_DIR / "realswin_y_test.mmap",
    }
    ...
    if ckpt_emb.exists():
        ...
        print("\n" + "=" * 68)
        print("  FASE 1 — EXTRACCIÓN EMBEDDINGS (Video Swin 3D congelado)")
        print("=" * 68)
        enc = load_swin_frozen()
    ...
    # FASE 2 — BASELINE
    # ══════════════════════════════════════════════════════════════════════════
    ckpt_base_pth = OUT_DIR / "realswin_baseline_best.pth"
    baseline_json = OUT_DIR / "realswin_baseline_result.json"
    ...
    if ckpt_baseline.exists() and baseline_json.exists():
        ...
        print("\n" + "=" * 68)
        print("  FASE 2 — BASELINE: Video Swin 3D congelado + MLP")
        print("=" * 68)
        set_seed(MLP_CFG["seed"])
        mlp = build_mlp()
        hist_base = train_on_embeddings(
            mlp, tr_emb, va_emb,
            lr=MLP_CFG["lr"], epochs=MLP_CFG["epochs"],
            patience=MLP_CFG["patience"], ckpt_path=ckpt_base_pth,
            label="BASELINE")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # FASE 3 — LORA+MLP end-to-end
    # ══════════════════════════════════════════════════════════════════════════
    ckpt_lora_pth = OUT_DIR / "realswin_lora_best.pth"
    lora_json     = OUT_DIR / "realswin_lora_result.json"
    ...
    if ckpt_lora.exists() and lora_json.exists():
        ...
        print("\n" + "=" * 68)
        print("  FASE 3 — LORA+MLP: Video Swin 3D + LoRA end-to-end")
        print(f"  r={LORA_CFG['rank']} | α={LORA_CFG['alpha']} | "
              f"dropout={LORA_CFG['lora_dropout']} | target=MLP-fc(0,3) | lr={MLP_CFG['lr_e2e']}")
        print("=" * 68)
        set_seed(MLP_CFG["seed"])
        enc_lora, n_lora, n_total = load_swin_with_lora()
        mlp_lora  = build_mlp()
        ...
        hist_lora = train_endtoend(
            full, tr_vid, va_vid,
            lr=MLP_CFG["lr_e2e"], epochs=MLP_CFG["epochs"],
            patience=MLP_CFG["patience"], ckpt_path=ckpt_lora_pth,
            label="LORA+MLP")
    ...
    # FASE 4 — SOLO_LORA
    # ══════════════════════════════════════════════════════════════════════════
    ckpt_sl_pth    = OUT_DIR / "realswin_solo_lora_best.pth"
    solo_lora_json = OUT_DIR / "realswin_solo_lora_result.json"
    ...
    if ckpt_solo_lora.exists() and solo_lora_json.exists():
        ...
        print("  Fix: LoRALinear en MLP-fc (FFN) → forward() invocado, grad_fn garantizado")
        print("=" * 68)
        set_seed(MLP_CFG["seed"])
        enc_sl, n_lora, n_total = load_swin_with_lora()
        mlp_sl = build_mlp()
        mlp_sl.load_state_dict(torch.load(ckpt_base_pth, map_location=DEVICE))
        for p in mlp_sl.parameters():
            p.requires_grad = False
        print("  MLP cargado desde baseline y congelado.")
        full_sl = FullPipeline(enc_sl, mlp_sl).to(DEVICE)
        tr_vid, va_vid, _ = make_vid_loaders(
            df_train, df_val, df_test, MLP_CFG["batch_vid"])
        hist_solo = train_endtoend_solo_lora(
            full_sl, tr_vid, va_vid,
            lr=MLP_CFG["lr_e2e"], epochs=MLP_CFG["epochs"],
            patience=MLP_CFG["patience"], ckpt_path=ckpt_sl_pth,
            label="SOLO_LORA")
    ...
    # LoRA+MLP
    enc_eval, _, _ = load_swin_with_lora()
    mlp_eval       = build_mlp()
    ...
    # Solo LoRA
    enc_sl, _, _ = load_swin_with_lora()
    mlp_sl       = build_mlp()
    full_sl      = FullPipeline(enc_sl, mlp_sl).to(DEVICE)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    final = {
        "codificador":    "VideoSwin3D (torchvision)",
        "checkpoint":     "Swin3D_B_Weights.KINETICS400_IMAGENET22K_V1",
        "frames_clip":    T,
        "emb_dim":        EMB_DIM,
        "config_mlp":     MLP_CFG,
    ...
    (OUT_DIR / "realswin_results.json").write_text(json.dumps(final, indent=2))
    ...
    for cfg, m in [("BASELINE", m_base), ("LORA+MLP", m_lora),
        ...
        print(f"  {cfg:<12} {m['AUC']:>6.4f} {m['F1']:>6.4f} "
              ...
              f"{m['FAR']:>6.4f}")
    print(f"\n  FPS: {op['fps']:.1f} | ms/frame: {op['ms_frame']:.2f}")

    print("\n  Generando gráficos...")
    histories    = {"BASELINE": hist_base,
                    "LORA+MLP": hist_lora, "SOLO_LORA": hist_solo}
    test_results = {"BASELINE": (p_base, l_base, m_base),
    ...
    print(f"\n  ✅ Pipeline completado.")
    print(f"  Resultados : {OUT_DIR}/realswin_results.json")
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 68)
    print("  Video Swin 3D (torchvision) — 3 experimentos (igual que I3D)")
    print(f"  MLP : fc | hidden=128 | dropout=0.3 | lr=1e-3")
    print(f"  LoRA: r={LORA_CFG['rank']} | α={LORA_CFG['alpha']} | "
          f"dropout={LORA_CFG['lora_dropout']} | target=MLP-fc(FFN) | lr=1e-4")
    print(f"  T={T} frames | EMB_DIM={EMB_DIM} | Swin3D-Base Kinetics400")
    print("=" * 68)
