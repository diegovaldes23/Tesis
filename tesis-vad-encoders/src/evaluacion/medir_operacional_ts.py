"""
medir_operacional_ts.py
─────────────────────────────────────────────────────────────
Script independiente para medir GFLOPs y FPS del pipeline
TimeSformer sin re-ejecutar el entrenamiento ni la evaluación.

Requiere que ya existan los checkpoints en processed/ts_results/
─────────────────────────────────────────────────────────────
Ejecutar:
    python3 medir_operacional_ts.py
"""
...
# ── Configuración — debe coincidir con timesformer_v3.py ─────────────────────
DEVICE          = torch.device("cuda" if torch.cuda.is_available() else "cpu")
TIMESFORMER_CKPT= "facebook/timesformer-base-finetuned-k400"
OUT_DIR         = Path("processed/ts_results")
T               = 8
IMG_SIZE        = 224
EMB_DIM         = 768
FRAMES_REAL     = 32
...
# ── Checkpoints ───────────────────────────────────────────────────────────────
CKPT_BASE = OUT_DIR / "ts_baseline_best.pth"
CKPT_LORA = OUT_DIR / "ts_lora_best.pth"
CKPT_SL   = OUT_DIR / "ts_solo_lora_best.pth"
JSON_PATH = OUT_DIR / "ts_results.json"
...
# ── LoRA (igual que en timesformer_v3.py) ────────────────────────────────────
...
def apply_lora(model, rank, alpha, lora_dropout, target_mods=("qkv",)):
    to_replace = [name for name, mod in model.named_modules()
...
# ── MLP (igual que en timesformer_v3.py) ─────────────────────────────────────
...
def medir(encoder, mlp, label):
    ...
    try:
        ...
        class _P(nn.Module):
            def __init__(self, e, m): super().__init__(); self.e=e; self.m=m
            def forward(self, x):
                pv  = x.permute(0,2,1,3,4).contiguous()
                cls = self.e(pixel_values=pv).last_hidden_state[:,0,:]
                return self.m(cls)

        flops  = FlopCountAnalysis(_P(encoder, mlp), dummy)
...
print("="*60)
enc_base = TimesformerModel.from_pretrained(TIMESFORMER_CKPT).to(DEVICE)
for p in enc_base.parameters():
    p.requires_grad = False
enc_base.eval()

mlp_base = AnomalyMLP(EMB_DIM, MLP_CFG["hidden_dim"],
                      MLP_CFG["dropout"]).to(DEVICE)
mlp_base.load_state_dict(torch.load(CKPT_BASE, map_location=DEVICE))

op_base = medir(enc_base, mlp_base, "BASELINE")
...
print("="*60)
enc_lora = TimesformerModel.from_pretrained(TIMESFORMER_CKPT).to(DEVICE)
for p in enc_lora.parameters():
    p.requires_grad = False
enc_lora = apply_lora(enc_lora, **{k: LORA_CFG[k] for k in
           ["rank","alpha","lora_dropout","target_mods"]})

mlp_lora = AnomalyMLP(EMB_DIM, MLP_CFG["hidden_dim"],
                      MLP_CFG["dropout"]).to(DEVICE)

# Cargar pesos del checkpoint completo (FullPipeline)
ckpt_state = torch.load(CKPT_LORA, map_location=DEVICE)
enc_keys = {k.replace("encoder.", "", 1): v
            for k, v in ckpt_state.items() if k.startswith("encoder.")}
mlp_keys = {k.replace("mlp.", "", 1): v
            for k, v in ckpt_state.items() if k.startswith("mlp.")}
enc_lora.load_state_dict(enc_keys, strict=False)
mlp_lora.load_state_dict(mlp_keys)

op_lora = medir(enc_lora, mlp_lora, "LORA+MLP")
del enc_lora, mlp_lora
if DEVICE.type == "cuda": torch.cuda.empty_cache()


# ── SOLO LORA: encoder con LoRA + MLP congelado ──────────────────────────────
print("\n" + "="*60)
print("  SOLO_LORA — encoder con LoRA + MLP")
print("="*60)
enc_sl = TimesformerModel.from_pretrained(TIMESFORMER_CKPT).to(DEVICE)
for p in enc_sl.parameters():
    p.requires_grad = False
enc_sl = apply_lora(enc_sl, **{k: LORA_CFG[k] for k in
         ["rank","alpha","lora_dropout","target_mods"]})

mlp_sl = AnomalyMLP(EMB_DIM, MLP_CFG["hidden_dim"],
                    MLP_CFG["dropout"]).to(DEVICE)

ckpt_state = torch.load(CKPT_SL, map_location=DEVICE)
enc_keys = {k.replace("encoder.", "", 1): v
            for k, v in ckpt_state.items() if k.startswith("encoder.")}
mlp_keys = {k.replace("mlp.", "", 1): v
            for k, v in ckpt_state.items() if k.startswith("mlp.")}
enc_sl.load_state_dict(enc_keys, strict=False)
mlp_sl.load_state_dict(mlp_keys)

op_solo = medir(enc_sl, mlp_sl, "SOLO_LORA")
del enc_sl, mlp_sl
if DEVICE.type == "cuda": torch.cuda.empty_cache()
