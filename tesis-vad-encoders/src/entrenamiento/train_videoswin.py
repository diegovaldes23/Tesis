...
=============================================================================
  ...
  Configuración LoRA (calibrada sobre I3D):
    ...
    Capas objetivo: attn.qkv dentro de SwinTransformerBlock3d
...
MLP_CFG = {
    ...
    "patience":   8,
    "batch_size": 64,
    "batch_vid":  4,    # conservador: 32 frames × 4 clips = 128 frames en VRAM
    "seed":       42,
}
...
LORA_CFG = {
    ...
    "alpha":        8,
    "lora_dropout": 0.10,
    "target_mods":  ("qkv",),  # SwinTransformerBlock3d usa attn.qkv
}

# ── Parámetros de video ───────────────────────────────────────────────────────
T           = 32      # Video Swin usa 32 frames (igual que I3D)
IMG_SIZE    = 224
NUM_WORKERS = 4
...
def apply_lora(model, rank, alpha, lora_dropout, target_mods=("qkv",)):
    """
    Aplica LoRA reemplazando capas Linear cuyo nombre hoja coincida
    con target_mods. En torchvision Swin3D las capas qkv son:
    features.N.N.attn.qkv  [dim → 3*dim]
    """
    to_replace = []
    ...
    if len(to_replace) == 0:
        print("  [WARN] No se encontraron capas exactas, buscando por substring...")
        for name, module in model.named_modules():
            if not isinstance(module, nn.Linear):
                continue
    ...
    if len(to_replace) == 0:
        print("  Capas Linear disponibles (primeras 30):")
        count = 0
...
def train_endtoend(full_model, tr_ldr, va_ldr, lr, epochs, patience,
                   ckpt_path, label):
    """Entrena LoRA+MLP end-to-end. Swin3D es nativo PyTorch — sin problemas de grad."""
    crit     = nn.BCEWithLogitsLoss()
...
def train_endtoend_solo_lora(full_model, tr_ldr, va_ldr, lr, epochs,
    ...
    """
    Solo LoRA: MLP congelado (eval), solo LoRA aprende.
    Swin3D es PyTorch nativo — el grafo de autograd funciona correctamente
    sin necesidad de hooks ni requires_grad_(True) en el input.
    """
    crit     = nn.BCEWithLogitsLoss()
    opt      = torch.optim.Adam(
        [p for p in full_model.parameters() if p.requires_grad], lr=lr)
...
def run_pipeline():
    ...
    if ckpt_solo_lora.exists() and solo_lora_json.exists():
        ...
        print("  Nota: Swin3D es PyTorch nativo — gradientes fluyen correctamente")
        print("=" * 68)
        set_seed(MLP_CFG["seed"])
...
if __name__ == "__main__":
    ...
    print(f"  LoRA: r=4 | α=8 | dropout=0.10 | target=qkv | lr=1e-4")
