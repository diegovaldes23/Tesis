...
class LiveInfer:
    def __init__(self, ) -> None:
        ...
        # ===== NUEVO: seleccionar dispositivo automáticamente =====
        ...
        self.frame_placeholder_ids = torch.tensor(
            ...
            device=self.device
        ).repeat(self.model.config.frame_num_tokens).reshape(1, -1)

        # generation
        self.system_prompt = args.system_prompt
        self.inplace_output_ids = torch.zeros(
            1, 100, device=self.device, dtype=torch.long
        )
    ...
    def _call_for_response(self, video_time, query):
        ...
        response = f'(Video Time = {video_time}s) Assistant:' + self.tokenizer.decode(
            output_ids[0],
    ...
    def input_query_stream(self, query, history=None, video_time=None):
        ...
        if not self.past_key_values:
            return (
                f'(NOTE: No video stream here. Please select or upload a video. '
                f'Then the assistant will answer "{query} (at {self.video_time}s)" in the video stream)'
            )
        return (
            f'(NOTE: Received "{query}" (at {self.video_time}s). '
            f'Please wait until previous frames have been processed)'
        )
    ...
    def input_video_stream(self, video_time):
        ...
        if frame_idx > self.last_frame_idx:
            ...
            frames_embeds = self.model.visual_embed(
                self.video_tensor[ranger]
            ).split(self.frame_num_tokens)
